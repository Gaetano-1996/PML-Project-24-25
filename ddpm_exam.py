...
class DDPM_low_discrepancy(nn.Module):
    ...
    def __init__(self, network, T=100, beta_1=1e-4, beta_T=2e-2, ld_sampler = "simple"):
        """
        Initialize Denoising Diffusion Probabilistic Model
...
def compute_fid(generated_images_dir = "./generated_images",
    ...
    print(f"Evaluation on {eval_batches} batches of size {batch_size}.")
    print(f"Model: Inception V3 with feature dimension {feature_dim}.")

    print("Loading real data into FID object...")
    # Loading real images to FID object
    for batch_idx, (data, _) in enumerate(dataloader_eval):
        fid.update(data, is_real=True)
        if eval_batches is not None:
            print(f" Batch {batch_idx + 1} / {eval_batches} loaded.")
        else:
            print(f" Batch {batch_idx + 1} / {len(dataloader_gen)} loaded.")

        if eval_batches is not None and batch_idx == eval_batches - 1:
            break

    print("Loading generated data into FID object...")
